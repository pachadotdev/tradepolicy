...
If the reader has not used R before, or to be more precise, has only fitted a few regressions without much practice on transforming and cleaning data before, please check chapters 5 and 18 from @wickham2016r.
...
Now the model is almost ready! Now we only need to stick to the methodology from @yotov2016advanced and cluster the standard errors by country pair (see the note on page 42, it is imperative). This note is not straightforward to replicate in R and requires additional work.

The *tradepolicy* package provides a nice function to do this and more. Please read
the documentation of the package and look for the `tp_summary_app_1()` function, it
summarises the model in the exact way as reported in the book by providing:

The *tradepolicy* package provides a nice function to do this and more. Please read the documentation of the package and look for the `tp_summary_app_1()` function, it exactly summarises the model as reported in the book by providing:
...
`fepois()` does not carry a copy of its training data by default besides providing faster fitting for models with fixed effects. This is not the case in base R, where `glm()` objects include this data, increasing the model's size, but this does not affect the model's predictions and can be changed as the user needs it [@trimmingfat].
...
Please notice that the previous summary intentionally does not show time exporter/importer fixed effects.
...
Notice that, unlike the previous section, if we had fitted the model in base R, with `glm()`, we would have needed to write a different formula.

```r
log_trade ~ 0 + log_dist_1986 + ... + clny + exp_year + imp_year
```

In which case, the initial zero means not to include a global constant besides the fixed effects. The fixed effect variables appear with regular plus signs, without a vertical bar to indicate them.
