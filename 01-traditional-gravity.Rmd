...
Provided that the datasets use 3 GB on disk, we opted to use a local SQL database ([DuckDB](https://duckdb.org)) instead of spreadsheets or even native R files. If you are not familiar with SQL, do not worry because we provide the function `tp_data()`, so that `tp_data("ch1_application1")` goes to the database and returns the data for this exercise.
...
the documentation of the package and look the `tp_model_summary()` function, it
...
Software such as Stata, without dedicated functions, reports an incorrect $R^2$ for PPML model, it actually reports a pseudo-$R^2$. To construct a proper $R^2$, `tp_model_summary()` takes the correlation between actual and predicted trade flows.
