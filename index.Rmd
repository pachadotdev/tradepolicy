...
The current edition uses *fixest* reduced the fitting for all the models by around 95%. The book's first edition takes around three hours to fit all the models and produce the final HTML. The same process now takes around ten minutes, and we hope these time efficiencies benefit the readers.

While writing this second edition, a series of bugs were detected and reported to improve *fixest*. For example, consider [this](https://github.com/lrberge/fixest/issues/286) error when having more than two fixed effects columns.

The possibility of comparing with base R, reporting and getting fixes from the packages' authors with transparent communication is a tremendous advantage from Open Source.

An additional improvement was to add additional explanations to the codes and review all the text grammar based on the feedback received after publishing the first edition.
