...
```{r ch1_app_1_data_1}
ch1_application1 <- agtpa_applications[, .(exporter, importer, pair_id, year, trade, dist, cntg, 
    lang, clny)][year %in% seq(1986, 2006, 4)]
```

For step 2, this can be divided in parts, starting with the log transformation of trade and distance.

```{r ch1_app_1_data_2}
ch1_application1[, `:=`(log_trade = log(trade), log_dist = log(dist))]
```
...
```{r ch1_app_1_data_3}
# Create Yit
ch1_application1[, c("y", "log_y") := .(sum(trade), log(sum(trade))), by = .(exporter, year)]

# Create Eit
ch1_application1[, c("e", "log_e") := .(sum(trade), log(sum(trade))), by = .(importer, year)]
```
...
```{r ch1_app_1_data_4}
# Replicate total_e
ch1_application1[, total_e := sum(e), by = .(exporter, year)]
ch1_application1[, total_e := max(total_e), by = year]

# Replicate rem_exp
ch1_application1[, c("remoteness_exp", "log_remoteness_exp") :=
  .(sum(dist * total_e / e), log(sum(dist * total_e / e))), by = .(exporter, year)]

# Replicate total_y
ch1_application1[, total_y := sum(y), by = .(importer, year)]
ch1_application1[, total_y := max(total_y), by = year]

# Replicate rem_imp
ch1_application1[, c("remoteness_imp", "log_remoteness_imp") :=
  .(sum(dist / (y / total_y)), log(sum(dist / (y / total_y)))), by = .(importer, year)]
```
...
ch1_application1[, c("exp_year", "imp_year") := .(paste0(exporter, year), paste0(importer, year))]
```
...
```{r ch1_app_1_data_6}
# ch1_application1 <- ch1_application1[exporter != importer]
```
...
fit_ols <- felm(
  ...
  data = ch1_application1[trade > 0 & exporter != importer] # commented out step 3
)
...
```{r ch1_app_1_ols_4}
ch1_application1_ols <- ch1_application1[trade > 0 & exporter != importer]
ch1_application1_ols[, trade_hat_sq := (predict(fit_ols, newdata = ch1_application1_ols))^2]
...
ch1_application1_ols_remoteness <- ch1_application1[trade > 0 & exporter != importer]
ch1_application1_ols_remoteness[, trade_hat_sq := (predict(fit_ols_remoteness, newdata = ch1_application1_ols_remoteness))^2]
...
```{r ch1_app_1_fe_2}
ch1_application1_fe <- ch1_application1[trade > 0 & exporter != importer]
ch1_application1_fe[, trade_hat_sq := (predict(fit_fe, newdata = ch1_application1_fe))^2]
...
fit_ppml <- fepoisson(trade ~ log_dist + cntg + lang + clny | exp_year + imp_year | pair_id,
  data = ch1_application1[exporter != importer] # commented out step 3
)
...
```{r ch1_app_1_ppml_3}
ch1_application1_ppml <- ch1_application1[exporter != importer]
ch1_application1_ppml[, trade_hat_sq := (predict(fit_ppml, type = "link", newdata = ch1_application1_ppml))^2]
...
ch1_application2[, c("exp_year", "imp_year", "year", "log_trade", "log_dist", "smctry") := .(
  paste0(exporter, year),
  paste0(importer, year),
  paste0("log_dist_", year),
  log(trade),
  log(dist),
  fifelse(importer != exporter, 0, 1)
)]

# PPML specific variables (depend on log_dist and smctry computed above)
ch1_application2[, c("log_dist_intra", "intra_pair") := .(
  log_dist * smctry,
  fifelse(exporter == importer, exporter, "inter")
)]

ch1_application2 <- dcast(
  ch1_application2,
  exporter + importer + pair_id + trade + dist + cntg + lang + clny +
    exp_year + imp_year + log_trade + smctry + log_dist_intra + intra_pair ~ year,
  value.var = "log_dist",
  fill = 0
)

dist_cols <- paste0("log_dist_", seq(1986, 2006, 4))

ch1_application2[, (dist_cols) := lapply(.SD, \(x) x * (1 - smctry)), .SDcols = dist_cols]
```

The `\(x)` syntax provides a shortcut to avoid repetition of the form
...
fit_ols_distance_puzzle <- felm(
  log_trade ~ log_dist_1986 + log_dist_1990 + log_dist_1994 +
    ...
    imp_year | pair_id,
  data = ch1_application2[importer != exporter & trade > 0]
)
...
fit_ppml_distance_puzzle <- fepoisson(
  ...
  data = ch1_application2[importer != exporter]
)
...
```{r ch1_app_3_data_1}
ch1_application3 <- agtpa_applications[year %in% seq(1986, 2006, 4)]

ch1_application3[, c("exp_year", "imp_year", "year", "log_trade", "log_dist",
                      "intl_brdr", "intl_brdr_2", "pair_id_2") := .(
  paste0(exporter, year),
  paste0(importer, year),
  paste0("intl_border_", year),
  log(trade),
  log(dist),
  fifelse(exporter == importer, as.character(pair_id), "inter"),
  fifelse(exporter == importer, 0, 1),
  fifelse(exporter == importer, "0-intra", as.character(pair_id))
)]

ch1_application3 <- dcast(ch1_application3, ... ~ year, value.var = "intl_brdr_2", fill = 0)
```
...
```{r ch1_app_3_data_2}
ch1_application3[, sum_trade := sum(trade), by = pair_id]
```
...
fit_ols_rta <- felm(
  ...
  data = ch1_application3[importer != exporter & trade > 0]
)
...
fit_ppml_rta <- fepoisson(
  ...
  data = ch1_application3[importer != exporter]
)
...
fit_endg_rta <- fepoisson(
  ...
  data = ch1_application3[sum_trade > 0]
)
