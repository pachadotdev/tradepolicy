...
We run the loop, which cannot be divided into smaller pieces because the step $N$ depends on the step $N-1$.
...
Box #1 from page 113 in @yotov2016advanced can be divided into smaller chunks. We start by filtering to keep the observation from 1994, and then we compute the trade costs ($\overline{t_{ij}}$ and $t_{ij}^{BLN}$) from the internal dyads fixed effects and the estimated RTA coefficient.
