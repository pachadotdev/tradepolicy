...
For now, we will concentrate on the fitted values and shall ignore the clustered standard errors in the next paragraphs, but still we can show the robust estimation by using the `tradepolicy_clustered_glm()` function:

```{r ch2_app1_baseline_2}
tradepolicy_clustered_glm(fit_baseline_app1$formula, ch2_application1)
```
