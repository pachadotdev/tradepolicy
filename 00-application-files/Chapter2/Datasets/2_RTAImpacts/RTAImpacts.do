...
* Close and create log	
	capture log close
	log using "Exercises/2_RTAImpacts/Results/RTAImpacts.log", text replace
...
* Open the database according to the Stata version you are using
	use "Datasets/Chapter2Exercise2.dta", clear
...
* Create a specific dummy for NAFTA
		generate NAFTA = 0
...
* Save data
	save "Datasets/2_RTAImpacts.dta", replace

	
****************************** EXERCISE 1 PART (i) *****************************
* Allow for the partial effects of NAFTA to be different than for the partial
* effects of other RTAs. Did the results confirm the expectations for stronger
* effects for NAFTA as compared to the effects of all other RTAs?

	* Estimate the gravity model with a specific variable for NAFTA 
		ppml trade PAIR_FE1-PAIR_FE$NTij_1 EXPORTER_TIME_FE* IMPORTER_TIME_FE1-IMPORTER_TIME_FE$NT_yr RTA NAFTA, iter(30) noconst
...
***************************** EXERCISE 1 PART (ii) *****************************
* Use the NAFTA-specific estimates from part A to obtain general equilibrium 
* effects of the NAFTA. Discuss the results and compare them with the main 
* estimates from Application 2. 	
	...
	*			indexes
		...
		* trade or zero trade
			...
			* Alternatively recall the results of the gravity model obtained above 
				...
				* Note that the nlcom command is particularly slow.
				* nlcom (exp(_b[RTA])-1) * 100
				scalar RTA_est = _b[RTA]	
				scalar NAFTA_est = _b[NAFTA]	
			...
			* Stage 2: Regress the estimates of pair fixed effects on gravity variables and country fixed effects
				...
				* standard gravity regression
					...
					replace tij_bln = tij_bar * exp(RTA_est*RTA+ NAFTA_est*NAFTA) if tij_bln == .	
	...
	* associated with the NAFTA
	generate NAFTA_CTF = 0	
		...
		* constraints associated with the counterfactual scenario
		generate tij_CFL = tij_bar * exp(RTA_est * RTA + NAFTA_est * NAFTA_CTF) 
...
* Step III: Solve the counterfactual model
	...
	* Save the conditional and general equilibrium effects results		
	save "Exercises/2_RTAImpacts/Results/FULLGE.dta", replace


* Step IV: Collect, construct, and report indexes of interest
	use "Exercises/2_RTAImpacts/Results/FULLGE.dta", clear
	...
	save "Exercises/2_RTAImpacts/Results/FULL_PROD.dta", replace
	...
	* Construct the percentage changes on import/consumption side
	...
	save "Exercises/2_RTAImpacts/Results/FULL_CONS.dta", replace

	* Merge the general equilibrium results from the production and consumption
	* sides
	use "Exercises/2_RTAImpacts/Results/FULL_PROD.dta", clear
		joinby country using "Exercises/2_RTAImpacts/Results/FULL_CONS.dta"
	...
	* Export the results in Excel
		export excel using "Exercises/2_RTAImpacts/Results/FULL.xls", firstrow(variables) replace


**************************** EXERCISE 1 PART (iii) *****************************
* Obtain the estimates from the Application 2 to ex ante simulate the effects of
* a potential RTA of your choice by focusing on the last year in the sample	
	
	* Repeat the commands above by defining a specific variable for the chosen
	* RTA and replacing the variable NAFTA with that variable
